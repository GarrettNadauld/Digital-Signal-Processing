...
float naive_interpolation(float x, float *h, uint16_t L);
float naive_decimation(float x, float *h, uint16_t M, uint16_t *count);

UTEST(ece5210_polyphase, polyphase_components)
{
    init_firwin();

    int ret = system("python test_utils/ece5210_polyphase_poly.py");
    ...
    for (uint16_t i = 0; i < h_poly_sol->size; i++)
    {
        ...
        diff += fabs((double)h_poly[i] - (double)h_poly_sol->data[i]);
    }
...
//UTEST(ece5210_polyphase, decimate_accuracy)
//{
//    init_firwin();
//
//    int ret = system("python test_utils/ece5210_polyphase_dec.py");
...
//    {
//        y_my->data[i] = polyphase_decimation(x->data[i], h_poly, DECIMATE);
//    }
//
//    double diff = 0;
//
...
//    {
//        diff += fabs((double)y_my->data[DECIMATE * i] - (double)y_sol->data[i]);
//    }
//    double error = diff / y_sol->size;
//
//    printf("Mean Absolute Error: %f\n", error); // Print out the mean absolute error
//
//    // Print out additional information if needed
...
//    / Print out the two arrays side by side
//    printf("Index\tComputed\tExpected\n"); // Header
//    for (uint16_t i = 0; i < y_sol->size; i++) {
//        printf("%5d\t%8.4f\t%8.4f\n", i, y_my->data[DECIMATE * i], y_sol->data[i]);
//    }
...
UTEST(ece5210_polyphase, decimate_time)
{
    init_firwin();
    uint16_t count = 0;
    ...
    for (uint32_t i = 0; i < x->size; i++)
    {
        polyphase_decimation(x->data[i], h_poly, DECIMATE);
    }
    end_time = clock();

    double poly_time = ((double)(end_time - start_time) / CLOCKS_PER_SEC);

    ASSERT_LT(poly_time / naive_time, 0.15);

    free(x);
    x = NULL;
}

UTEST(ece5210_polyphase, interpolate_accuracy)
{
    init_firwin();

    int ret = system("python test_utils/ece5210_polyphase_int.py");
    char filename_x[] = "x_out.mtx";
    char filename_y[] = "y_out.mtx";
    ...
    for (uint16_t i = 0; i < y_sol->size; i++)
    {
        if (i % L == 0)
        {
            y_my->data[i] = polyphase_interpolation(x->data[i / L], h_poly, L);
        }
        else
        {
            y_my->data[i] = polyphase_interpolation(0, h_poly, L);
        }
    }
    ...
    for (uint16_t i = 0; i < y_sol->size; i++)
    {
        diff += fabs((double)y_my->data[i] - (double)y_sol->data[i]);
    }

    double error = diff / y_sol->size;

//    printf("Index\tMy Array\tSolution Array\n"); // Header for the table
//        for (uint16_t i = 0; i < y_sol->size/100; i++) {
//        printf("%d\t%f\t%f\n", i, y_my->data[i], y_sol->data[i]);
//        }
...
UTEST(ece5210_polyphase, interpolate_time)
{
    init_firwin();
    ...
    for (uint32_t i = 0; i < x->size; i++)
    {
        polyphase_interpolation(x->data[i], h_poly, DECIMATE);
    }
    end_time = clock();
...
UTEST(ece5210_polyphase, total_system)
{
    init_firwin();

    int ret = system("python test_utils/ece5210_polyphase_total.py");
    char filename_x[] = "x_out.mtx";
    char filename_y[] = "y_out.mtx";
    ...
    array *y_my = initialize_array(y_sol->size);
    ...
    for (uint16_t i = 2000; i < y_sol->size; i++)
    {
        diff += fabs((double)y_my->data[i] - (double)y_sol->data[i]);
    }
...
array_f32 *initialize_array_f32(uint32_t size)
{
    array_f32 *out = malloc(sizeof(*out) + sizeof(float) * (size_t)size);
...
float fir_lazy(float x, float *h, float *state, uint16_t size)
{
    float out = 0.f;
...
float naive_interpolation(float x, float *h, uint16_t L)
{
    float out = fir_lazy(x, h, interp_state, NUM_TAPS);
...
float naive_decimation(float x, float *h, uint16_t M, uint16_t *count)
{
    float out = fir_lazy(x, h, dec_state, NUM_TAPS);
